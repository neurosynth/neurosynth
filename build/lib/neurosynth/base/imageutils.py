...
def get_sphere(coords, r=4, vox_dims=(2,2,2), dims=(91,109,91)):
	...
	r = float(r)
	ranges = [np.arange(-r/vox_dims[i], r/vox_dims[i]+0.01) for i in range(len(coords))]
	cube = np.vstack([row.ravel() for row in np.meshgrid(*ranges)])
...
def save_img(data, filename, volume, header=None):
	""" Save a vectorized image to file. """
	# data = data.astype(dtype)
	if not header:
		header = volume.get_header()
	header.set_data_dtype(data.dtype) # Avoids loss of precision
	img = nifti1.Nifti1Image(volume.unmask(data), None, header)
