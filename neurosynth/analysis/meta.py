...
def analyze_features(dataset, features, image_type=None, threshold=0.001, q=0.05, save=None):
  ...
  if image_type is None:
    image_type = 'pFgA_z'  # Default to use reverse inference z-score map
  if save is None:
...
class MetaAnalysis(object):
  ...
  def __init__(self, dataset, ids, ids2=None, **kwargs):
    ...
    prior_pF = kwargs.get('prior', 0.5)         # prior probability of feature being present
    min_studies = kwargs.get('min_studies', 0)  # min. number of studies for voxel inclusion
    ...
    pA = np.array((mt.data.sum(axis=1) + 1.0) / (n_mappables+2)).squeeze()
    ...
    if min_studies > 0:
      if isinstance(min_studies, int):
        min_studies = float(min_studies) / n_mappables  # Recalculate as proportion
